...
def _build_request(lat, lon, start_date, end_date):
    ...
    return OPEN_METEO_SOLAR_URL, params


def _request_irradiance(url, params, lat, lon, start_date, end_date):
    logger.info(
        f"Fetching irradiance data for ({lat}, {lon}) "
...
def fetch_irradiance(lat, lon, start_date, end_date):
    """
    Fetches hourly irradiance data from Open-Meteo for a single location.
